...
class SubscriptionPersistenceTestImpl extends SubscriptionPersistence[StateT[IO, ServiceState, *]] {
  override def addSubscription(subscription: NewSubscription): StateT[IO, ServiceState, Unit] =
    StateT { serviceState =>
      Subscription.Id
        .generate[IO]
        .map(id =>
          (serviceState.addSubscription(id, subscription.userId, subscription.repository), ())
        )
    }

  override def getSubscriptionsByRepository(
      repository: Repository
  ): StateT[IO, ServiceState, List[Subscription]] = StateT { serviceState =>
    IO(
      (
        serviceState,
        serviceState.subscriptionDbTable.filter(s =>
          s.repository == repository && s.deletedAt.isEmpty
        )
      )
    )
  }
  ...
  override def getSubscriptionsByUserId(
    ...
    StateT { serviceState =>
      IO(serviceState, serviceState.subscriptionDbTable.filter(_.user == userId))
    }
  ...
  override def removeSubscription(
    ...
    StateT { serviceState =>
      IO(serviceState.cancelSubscription(userId, repository), ())
    }
}
